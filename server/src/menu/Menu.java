...
public class Menu {
   ...
   public void showMenu(BufferedReader inStream, PrintStream outStream,
         GameSetting settings) {
      for (Option option : options) {
         outStream.print(option);

         if (this.name.equals(MenuName.DIFFICULTY_MENU)) {
            if (settings.getDifficulty().value
         ...
         for (Option option : options) {
            if (option.isValid(choiceInt)) {
               if (option.getNext() != null) { // Recursive case next
                  ...
                  if (this.name.equals(MenuName.DIFFICULTY_MENU)) { // Special
                                                                    // case
                                                                    // difficulty
                     gameHandler.getSettings().setDifficulty(
                           Difficulty.fromString(option.getOption()));
                     gameHandler.getOutStream()
                           .println("La difficulté a bien été modifiée");
                     return this;
                     ...
                     if (option.getOption().equalsIgnoreCase("Classique")) {
                        gameHandler
                              .setGame(new Classic(gameHandler.getSettings()));
                        return null;
                     }
                     if (option.getOption().equalsIgnoreCase("Essai en or")) {
                        gameHandler
                              .setGame(new Golden(gameHandler.getSettings()));
                        return null;
                     }
                     if (option.getOption()
                           .equalsIgnoreCase("Ruée aux points")) {
                        gameHandler.setGame(
                              new PointsRush(gameHandler.getSettings()));
                        return null;
                     }
                     ...
                     gameHandler.showLeaderBoard(
                           Difficulty.fromString(option.getOption()));
                     return this;
                  } else if (this.name.equals(MenuName.HELP_MENU)) {
                     if (option.getOption()
                           .equalsIgnoreCase("Les difficultés")) {
                     ...
                     if (option.getOption().equalsIgnoreCase("Classique")) {
                        gameHandler.getOutStream()
                              .println(Classic.getFullDescription());
                        return this;
                     }
                     if (option.getOption().equalsIgnoreCase("Essai en or")) {
                        gameHandler.getOutStream()
                              .println(Golden.getFullDescription());
                        return this;
                     }
                     if (option.getOption()
                           .equalsIgnoreCase("Ruée aux points")) {
                        gameHandler.getOutStream()
                              .println(PointsRush.getFullDescription());
                        return this;
                     }
                  }
               }
